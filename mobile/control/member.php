...
if('edit' == $opera)
{
    ...
    if($mobile == '')
    {
        ...
        $check_mobile = 'select `mobile` from '.$db->table('member').' where `mobile`=\''.$mobile.'\' and `account`<>\''.$account.'\'';
...
if('get_node' == $act)
{
    ...
    if($nodes)
    {
        foreach ($nodes as $node) {
            ...
            $node['name'] .= '[会员卡号:' . $node['account'] . ', 会员等级:' . $lang['level'][$node['level_id']] . ', 注册时间:' . date('Y-m-d H:i:s', $node['add_time']) . ']';
            $tree_nodes[] = $node;
        }
    }
...
if('view' == $act)
{
    ...
    $get_total = 'select count(*) from '.$db->table('member').$where;
